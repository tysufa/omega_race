...
class Player:
    ...
    def central_square_collision(self, central_square):
        if self.vaisseau_rect.colliderect(central_square):
            if self.vaisseau_rect.right >= central_square.left and self.vaisseau_rect.left <= central_square.right:
                self.velocity.x = -self.velocity.x
            if self.vaisseau_rect.bottom >= central_square.top and self.vaisseau_rect.top <= central_square.bottom:
                self.velocity.y = -self.velocity.y
