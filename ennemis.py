...
class ennemi:
    def __init__(self, x, y, WINDOW):
        ...
        self.height = tu[1]
        self.widht = tu[0]
        while self.x + 10 > (self.widht // 2 - self.widht // 6) and self.x - 10 < (
                self.widht // 2 + self.widht // 6) and self.y + 10 > (
                self.height // 2 - self.height // 6) and self.y - 10 < (self.height // 2 + self.height // 6):
            self.x = randint(0, tu[0])
            self.y = randint(0, tu[1])

    def colver(self):
        return self.y + 10 >= self.height or self.y - 10 <= 0 or (
                    self.x + 10 > (self.widht // 2 - self.widht // 6) and self.x - 10 < (
                        self.widht // 2 + self.widht // 6) and self.y + 10 > (
                                self.height // 2 - self.height // 6) and self.y - 10 < (
                                self.height // 2 + self.height // 6))  # essayer de récuperer les dimensions de la fenètre + essayer de prendre en compte le carré central

    def colhor(self):
        return self.x + 10 >= self.widht or self.x - 10 <= 0 or (
                    self.x + 10 > (self.widht // 2 - self.widht // 6) and self.x - 10 < (
                        self.widht // 2 + self.widht // 6) and self.y + 10 > (
                                self.height // 2 - self.height // 6) and self.y - 10 < (
